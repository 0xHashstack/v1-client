...
const BorrowModal = ({
	...
	const [currentTransactionStatus, setCurrentTransactionStatus] =
		useState('');
	const [collateralMarketIndex, setcollateralMarketIndex] =
		useState<Number>(-1);
	...
	useEffect(() => {
		setCurrentAvailableReserves(
			protocolStats[coinAlign?.indexOf(currentBorrowCoin)]
				?.availableReserves * 0.895
		);
			...
			if (currentCollateralCoin[0] === 'r') {
				...
				if (borrow?.transaction_hash) {
					...
					if (showToast == 'true') {
						...
						const trans_data = {
							transaction_hash:
								borrow?.transaction_hash.toString(),
							message: `Successfully borrowed : ${inputBorrowAmount} d${currentBorrowCoin}`,
							toastId: toastid,
							setCurrentTransactionStatus:
								setCurrentTransactionStatus,
						...
						<Box
							...
							<Box
								...
								<Box
									width='100%'
									color={`${
										rTokenAmount > walletBalance ? '#CF222E'
										: rTokenAmount < 0 ? '#CF222E'
										: (
											...
											'#CF222E'
										: rTokenAmount == 0 ? 'white'
										: '#00D395'
									}`}
									border={`${
										rTokenAmount > walletBalance ?
											'1px solid #CF222E'
										: rTokenAmount < 0 ? '1px solid #CF222E'
										: (
											process.env.NEXT_PUBLIC_NODE_ENV ==
												'mainnet' &&
											rTokenAmount > 0 &&
								...
								:	<Text
										...
										{(
											...
											'rToken Balance: ' +
											getBalance(currentCollateralCoin)
										:	'Wallet Balance: ' +
											((
												walletBalance
													.toFixed(5)
								...
								<Box
									...
									onClick={() => {
										if (transactionStarted) {
											...
											handleDropdownClick(
												'borrowModalBorrowMarketDropdown'
											);
										}
									}}
									as='button'>
								...
								<Box
									...
									border={`${
										...
										: (
											...
											'1px solid #CF222E'
										: isNaN(amount) ? '1px solid #CF222E'
										: amount > 0 ? '1px solid #00D395'
										: '1px solid var(--stroke-of-30, rgba(103, 109, 154, 0.30)) '
									}`}
									borderRadius='6px'
									display='flex'
							...
							<Card
								...
								<Text
									...
									<Text
										...
										{(
											!borrowAPRs ||
											borrowAPRs[currentBorrowAPR] == null
										) ?
											<Box pt='2px'>
												<Skeleton
													width='2.3rem'
