		...
		try {
			...
			if (val?.transaction_hash) {
				...
				posthog.capture('Repay Borrow Status', {
					...
					'Collateral Token': collateralBalance.substring(
						spaceIndex + 1
					),
				});
		...
		(
			...
			parseAmount(
				String(
					uint256.uint256ToBN(
						walletBalances[
							currentBorrowMarketCoin1.slice(1) as NativeToken
		...
		setwalletBalance2(
			walletBalances[collateralAsset]?.statusBalanceOf === 'success' ?
				parseAmount(
					String(
						uint256.uint256ToBN(
							walletBalances[collateralAsset]?.dataBalanceOf
								?.balance
						)
					),
					tokenDecimalsMap[collateralAsset]
		...
		try {
			...
			if (currentDapp == 'Jediswap') {
				...
				if (tradeMySwap?.transaction_hash) {
					...
					const trans_data = {
						transaction_hash:
							tradeMySwap?.transaction_hash.toString(),
						message: `Successfully traded for loan ID : ${swapLoanId}`,
						toastId: toastid,
						setCurrentTransactionStatus:
							setCurrentTransactionStatus,
		...
		try {
			if (currentDapp == 'Jediswap') {
				...
				if (mySwapLiquidity?.transaction_hash) {
					...
					const trans_data = {
						transaction_hash:
							mySwapLiquidity?.transaction_hash.toString(),
						// message: `You have successfully added Liquidity for loan ID : ${liquidityLoanId}`,
						message: `Successfully added Liquidity`,
						toastId: toastid,
						setCurrentTransactionStatus:
			...
			if (currentTokenSelected == 'rToken') {
				...
				if (addCollateral?.transaction_hash) {
					...
					if (addCollateral?.transaction_hash) {
						...
						const trans_data = {
							transaction_hash:
								addCollateral?.transaction_hash.toString(),
				...
				if (addCollateral?.transaction_hash) {
					...
					if (addCollateral?.transaction_hash) {
						...
						if (addCollateral?.transaction_hash) {
							...
							activeTransactions?.push(trans_data);
							posthog.capture(
								'Add Collateral Your Borrow Status',
								{
									Status: 'Success',
									'Loan id': currentBorrowId2,
									'Borrow Market': currentBorrowMarketCoin2,
							...
							<Box
								...
								<Text
									...
									:	numberFormatterPercentage(
											getBoostedApr(
												currentBorrowMarketCoin1.slice(
													1
												)
											) +
												getBoostedAprSupply(
						...
						<Box
																	...
																	return (
																		<Box
																			...
																			onClick={() => {
																				...
																				setCollateralAsset(
																					currentBorrowMarketCoin2.slice(
																						1
																					)
																				);
																				setRToken(
                            ...
                            and remain locked till the debt is repaid.'
										...
										{(
											...
											<Box
												onClick={() => {
													setCollateralTransactionStarted(
														true
													);
													posthog.capture(
