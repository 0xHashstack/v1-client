	...
	const handleBorrowMarketCoinChange = (id: string) => {
		...
		for (let i = 0; i < borrowIDCoinMap?.length; i++) {
			if (borrowIDCoinMap?.[i]?.id === id) {
				setCurrentBorrowMarketCoin(
					borrowIDCoinMap?.[i]?.name?.slice(1)
				);
				return;
			}
		}
												...
												return (
													<Box
														...
														<Box
															...
															display='flex'
															py='5px'
															px={`${(coin === currentSelectedCoin && poolHoverIndex === -1) || poolHoverIndex === index ? '1' : '5'}`}
															gap='1'
															bg={`${
																(
																	(coin ===
							...
							<Box
								onClick={() => {
									...
									if (transactionStarted == false) {
										posthog.capture(
											'Swap Modal Button Clicked Spend Borrow',
											{
												Clicked: true,
											}
										);
