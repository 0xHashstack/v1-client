		...
		if (userLoans && oraclePrices) {
			...
			for (const loan of userLoans) {
				const loan_ltv1 =
					...
					oraclePrices?.find(
						(val: any) => val?.name == loan?.underlyingMarket
					)?.price;
				const loan_ltv2 =
			...
			<Box
				...
				<Box>
					...
					<Text
						...
						color='#BDBFC1'
						mt='0.4rem'>
						Spend your borrowed funds on these Jediswap pools to
						receive the rewards.
					</Text>
					{userLoans?.length > 0 && (
						<Box
			...
			<Box
				...
				<Box>
					...
					<Text
						...
						color='#BDBFC1'
						mt='0.4rem'>
						Supply your funds on these markets to receive the
						rewards.
					</Text>
					<Box
						display='flex'
