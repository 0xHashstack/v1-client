...
const SupplyDashboard = ({
	...
	upper_bound = Math.min((userDeposits?.length || 6) - 1, upper_bound); // MOCK: fallback to 6 for demo
	// useEffect(() => {
	//   try {
	//     const supply = async () => {
	...
	useEffect(() => {
		...
		const mockSTRK = { token: 'STRK', rToken: 'rSTRK', rTokenAmountParsed: 230, rTokenFreeParsed: 230, rTokenLockedParsed: 0, rTokenStakedParsed: 0 };
		setSupplies([mockSTRK] as any);
		dispatch(setUsersFilteredSupply(1));
		setLoading(false);
	}, []);
