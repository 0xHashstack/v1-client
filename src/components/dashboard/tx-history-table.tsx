...
const TxHistoryTable = ({
  ...
  const [ lookingForTxs, setLookingForTxs ] = useState<{[key: string]: boolean}>({});
  ...
  return (
    <div className="table-responsive">
      ...
      <div style={{padding: '10px', textAlign: 'center', width: '100%'}}>
        {loading && !Object.keys(lookingForTxs).length ? <MySpinner/> : <span></span>}
        {
          Object.keys(lookingForTxs).filter(tx => lookingForTxs[tx]==false).map(tx => {
            let len = tx.length;
            return <MySpinner key={tx} text={`Waiting for ${tx.substring(0, 4)}...${tx.substring(len-4, len)} (~ 1 min)`}/>
          })
        }
      </div>
    </div>
