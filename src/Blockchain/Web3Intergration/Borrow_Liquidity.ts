...
async function Borrow_Liquidity_Integration(market:any,amount:any,rToken:any,rTokenAmount:any,recipient:any) {
    ...
    const par =  CallData.compile({
        _market: market,
        _amount: amount,
        _rtoken: rTokenAmount,
        _rtokenAmount: rTokenAmount,
        _recipient: recipient,
    })

    try {
       const result = await myTestContract.loan_request(par);
