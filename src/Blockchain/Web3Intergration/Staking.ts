...
const staking_address = "0xu23nbsd";

async function Staking_Integration(rToken:any,amount:any,receiver:any) {
    ...
    const par =  CallData.compile({
        _rtoken: rToken,
        _rTokenAmount: amount,
        _receiver: receiver,
    })

    try {
        const result = await myTestContract.stake(par);
...
async function Unstaking_Integration(rToken:any,amount:any,receiver:any) {
    ...
    try {
        const result = await myTestContract.unstake(par);
        await provider.waitForTransaction(result.transaction_hash);
