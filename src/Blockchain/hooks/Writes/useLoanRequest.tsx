...
const useLoanRequest = () => {
	...
	const [rTokenAmountCollateral, setRTokenAmountCollateral] =
		useState<number>(0);
	const balance = useSelector(selectNftBalance);

	// Collateral - native token Market
	const [collateralMarketNative, setCollateralMarketNative] =
		useState<NativeToken>('BTC');
	const [collateralAmountNative, setCollateralAmountNative] =
		useState<number>(0);
				...
				[
					...
					},
					{
						...
						calldata: [
							...
							tokenAddressMap[collateralMarketNative] || '',
							etherToWeiBN(
								collateralAmountNative as number,
								collateralMarketNative
							).toString(),
							0,
							account,
