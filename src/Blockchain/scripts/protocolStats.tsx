...
function parseProtocolStats(
  ...
  for (let i = 0; i < market_infos?.length; ++i) {
    ...
    let loan: IMarketInfo = {
      ...
      utilisationPerMarket: uint256.uint256ToBN(marketData?.utilisation_per_market).toString(),

      exchangeRateRtokenToUnderlying: uint256.uint256ToBN(marketData?.exchange_rate_rToken_to_asset).toString(),
      exchangeRateDTokenToUnderlying: uint256.uint256ToBN(marketData?.exchange_rate_dToken_to_asset).toString(),
      exchangeRateUnderlyingToRtoken: uint256.uint256ToBN(marketData?.exchange_rate_asset_to_rToken).toString(),
      exchangeRateUnderlyingToDtoken: uint256.uint256ToBN(marketData?.exchange_rate_asset_to_dToken).toString(),
...
export async function getProtocolStats() {
  ...
  return parseProtocolStats(
    res?.market_infos,
  );
}


function parseProtocolReserves(
  protocolReservesData: any,
): IProtocolReserves {
  let protocolReserves: IProtocolReserves = {
    totalReserves: uint256.uint256ToBN(protocolReservesData?.total_reserves).toString(),
    availableReserves: uint256.uint256ToBN(protocolReservesData?.available_reserves).toString(),
    avgAssetUtilisation: uint256.uint256ToBN(protocolReservesData?.avg_asset_utilisation).toString(),
  };
