...
const useTransactionHandler = () => {
	...
	useEffect(() => {
		...
		results?.forEach((transaction: any, idx) => {
			...
			const transaction_hxh =
				activeTransactions[transactions.length - 1]?.transaction_hash;
			if (transaction_status == 'ACCEPTED_ON_L2') {
				if (!toastHash.includes(transaction_hxh)) {
					if (
						data &&
						data.includes(
							activeTransactions[transactions.length - 1]
								?.uniqueID
						)
					) {
			...
			if (
				...
				toast.dismiss(
					activeTransactions?.[transactions.length - 1]?.toastId
				);
