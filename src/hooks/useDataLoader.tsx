...
const useDataLoader = () => {
	...
	const mySwapPoolsSupportedCount = useSelector(
		selectMySwapPoolsSupportedCount
	);
			...
			try {
				...
				Promise.allSettled([...promises]).then((val) => {
					...
					for (var j = 0; j < 6; j++) {
						...
						// ) {
						const responseTotal =
							responseC?.status != 'rejected' ?
								responseC?.value
							:	null;
						const response =
							responseA?.status != 'rejected' ?
								responseA?.value
							:	null;
						const responseApr =
							responseB?.status != 'rejected' ?
								responseB?.value
							:	null;
						...
						if (response && responseApr && responseTotal) {
							...
							for (var i = 0; i < response?.length; i++) {
								...
								supplyRates?.push(
									response?.[i]?.supplyRate / 100
								);
								borrowRates?.push(
									response?.[i]?.borrowRate / 100
								);
								supplyCounts?.push(
									response?.[i]?.supplyAccounts
								);
								borrowCounts?.push(
									response?.[i]?.borrowAccounts
								);
								utilRates?.push(response?.[i]?.utilRate / 100);
								rTokenExchangeRates?.push(
									1 /
										(response?.[i]?.rTokenExchangeRate /
											10000)
								);
								dTokenExchangeRates?.push(
									1 /
										(response?.[i]?.dTokenExchangeRate /
											10000)
								);
								totalTransactions?.push(
									response?.[i]?.totalTransactions
								);
								totalAccounts?.push(
									response?.[i]?.totalAccounts
								);
								aprs?.push(responseApr?.[i]?.APY);
								apys?.push(responseApr?.[i]?.APR);
								totalUrm?.push(
									responseTotal?.[i]?.totalPlatformURM / 100
								);
							}
			...
			const fetchFees = async () => {
				...
				Promise.allSettled([...promises]).then((val) => {
					const data = {
						supply:
							...
							:	null,
						stake:
							val?.[1]?.status == 'fulfilled' ?
								val?.[1]?.value
							:	0,
						unstake:
							val?.[2]?.status == 'fulfilled' ?
								val?.[2]?.value
							:	0,
						withdrawSupply:
							val?.[3]?.status == 'fulfilled' ?
								val?.[3]?.value
							:	0,
						borrow:
							val?.[4]?.status == 'fulfilled' ?
								val?.[4]?.value
							:	0,
						borrowTrade: 0.1,
						l3interaction:
							val?.[5]?.status == 'fulfilled' ?
								val?.[5]?.value
							:	0,
						repayLoan:
							val?.[6]?.status == 'fulfilled' ?
								val?.[6]?.value
							:	0,
					};
					const nft =
						val?.[7]?.status == 'fulfilled' ? val?.[7]?.value : 0;
					const nftMaxAmount =
				...
				if (process.env.NEXT_PUBLIC_NODE_ENV == 'mainnet') {
					...
					if (dataExisitingLink) {
						dispatch(
							setExisitingLink(dataExisitingLink?.data?.ref)
						);
					}
				}
			};
	...
	useEffect(() => {
		try {
			const fetchProtocolStats = async () => {
				...
				if (
					!dataStats ||
					(Array.isArray(dataStats) && dataStats?.length < 6)
				) {
					return;
				}
				...
				if (data) {
					...
					indexes.forEach((index: number) => {
						if (
							...
							if (index == 2 || index == 3) {
								if (
									supply?.[index]?.rTokenAmountParsed >
										0.000001 ||
									supply?.[index]?.rTokenFreeParsed >
										0.000001 ||
									supply?.[index]?.rTokenLockedParsed >
										0.000001 ||
									supply?.[index]?.rTokenStakedParsed >
										0.000001
								) {
									supplyCount++;
								}
				...
				Promise.allSettled([...promises]).then((val) => {
					...
					const maxdata = {
						rBTC:
							...
							:	0.00074,
						rETH:
							val?.[7]?.status == 'fulfilled' ?
								val?.[7]?.value
							:	0.012,
						rUSDT:
							val?.[8]?.status == 'fulfilled' ?
								val?.[8]?.value
							:	20,
						rUSDC:
							val?.[9]?.status == 'fulfilled' ?
								val?.[9]?.value
							:	20,
						rDAI:
							val?.[10]?.status == 'fulfilled' ?
								val?.[10]?.value
							:	20,
						rSTRK:
							val?.[11]?.status == 'fulfilled' ?
								val?.[11]?.value
							:	20,
					};
			...
			const fetchHealthFactor = async () => {
				...
				Promise.all([...promises]).then((val: any) => {
					const avgs = val.map((loneHealth: any, idx: number) => {
						return {
							loanHealth: loneHealth,
							loanId: userLoans[idx]?.loanId,
						};
					});
	...
	useEffect(() => {
		try {
			const fetchSupplyData = async () => {
				const data = dataDeposit?.map(
					(deposit: IDeposit, idx: number) => {
						const price = oraclePrices?.find(
							(oraclePrice: any) =>
								oraclePrice?.name == deposit?.token
						)?.price;
						const token_amount =
							deposit?.underlyingAssetAmountParsed;
						// const token_amount =
			...
			const fetchBorrowData = async () => {
				...
				for (let loan of userLoans) {
					...
					if (oraclePrice && exchangeRate) {
						let loanAmoungUnderlying =
							loan?.loanAmountParsed * exchangeRate;
				...
				if (oraclePrices && spendBalances && protocolStats) {
					for (var i = 0; i < spendBalances?.length; i++) {
						if (
							...
							let value =
								(protocolStats[i]?.totalBorrow -
									spendBalances[i]?.balance) *
		...
		try {
			const fetchData = async () => {
				...
				if (res?.data) {
					const count = getTransactionCount();
					dispatch(
						setJedistrkTokenAllocation(res?.data?.Jediswap_v1)
					);
	...
	useEffect(() => {
		const fetch = async () => {
			let arr: any = [];
			await Promise.all(
				userLoans.map(async (Borrow: any, idx: number) => {
					if (Borrow?.l3App === 'ZKLEND') {
						const usdspend = await getZklendusdSpendValue(
							Borrow?.loanId,
							Borrow?.loanMarket
						);
						const val = {
	...
	useEffect(() => {
		const fetchDataForCampaigns = async () => {
			const [epochDataRes, ccpRegisterRes, ccpSubmissionRes] =
				await Promise.all([
					axios
						.get(
					...
					axios
						...
						.catch((err) => null),
					axios
						.get(`https://hstk.fi/api/ccp/submission/${address}`)
						.catch((err) => null),
				]);
			if (epochDataRes) {
	...
	useEffect(() => {
		const fetchLeaderboardData = async () => {
			const [ccpLeaderBoardData, AirdorpLeaderBoardData] =
				await Promise.all([
					axios
						.get('https://hstk.fi/api/ccp/submissions')
						.catch((err) => null),
					axios
						.get('https://hstk.fi/api/leaderboard')
						.catch((err) => null),
				]);
